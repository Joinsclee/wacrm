    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error('Aún no hay ningún agente configurado — primero completa la configuración.');
        } else {
        ...
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            <p className="mt-1 text-xs">
              ...
              bot de respuesta automática, incluida la transferencia.
            </p>
            {onGoToSetup && (
              <Button
                variant="link"
              ...
              >
                ¿Aún no está configurado? Ir a la configuración <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
