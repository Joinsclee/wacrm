    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip="Cantidad de todas las oportunidades de este embudo que no están marcadas como Perdidas. Las ganadas también se incluyen."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label="Valor del embudo"
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip="Suma de los valores de todas las oportunidades de este embudo, excluyendo las marcadas como Perdidas."
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label="Valor promedio"
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip="El Valor del embudo dividido entre el Total de oportunidades: el valor promedio de una oportunidad no perdida."
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label="Valor ponderado"
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip="Ingresos esperados: el valor de cada oportunidad abierta × la probabilidad de su etapa. Primera etapa ≈ 10%, las etapas avanzan hasta 90%, Ganada = 100%. Las oportunidades perdidas se excluyen."
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-primary" />}
          label="Ganadas este mes"
          value={String(stats.wonThisMonth)}
          tooltip="Oportunidades marcadas como Ganadas desde el primer día del mes actual."
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-400" />}
          label="Perdidas este mes"
          value={String(stats.lostThisMonth)}
          tooltip="Oportunidades marcadas como Perdidas desde el primer día del mes actual."
        />
      </div>
    </TooltipProvider>
  );
