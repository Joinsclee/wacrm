...
export function PipelineSettings({
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error("Primero mueve o elimina las oportunidades de esta etapa");
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-popover border-border max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">Administrar embudo</DialogTitle>
        </DialogHeader>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Esto archivará todas las oportunidades de este embudo. No se
                  puede deshacer.
                </p>
              </div>
            </div>
