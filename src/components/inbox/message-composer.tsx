...
export function MessageComposer({
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === "ai_not_configured") {
          toast.error("La IA aún no está configurada — actívala en Ajustes → Asistente IA.");
        } else {
          toast.error(data.error ?? "No se pudo redactar una respuesta.");
        }
        return;
      }
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(
          `El archivo pesa ${(file.size / 1024 / 1024).toFixed(1)} MB — el límite de ${kind} es ${Math.round(
            max / 1024 / 1024,
          )} MB.`,
        );
        return;
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error("Este navegador no admite la grabación de voz.");
      return;
    }
    try {
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-xs text-amber-400">
            La sesión de 24 horas expiró. Usa una plantilla para retomar el contacto.
          </p>
          <Button
            variant="ghost"
            size="sm"
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <GatedButton
            ...
            gateReason="enviar mensajes"
            disabled={drafting}
            title={readOnly ? undefined : "Redactar una respuesta con IA"}
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ? "Solo lectura — puedes ver pero no responder"
                : sessionExpired
