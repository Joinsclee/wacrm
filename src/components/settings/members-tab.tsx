...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Administrador', hint: 'Gestiona miembros + todo' },
  { value: 'agent', label: 'Agente', hint: 'Usa las funciones; sin ajustes' },
  { value: 'viewer', label: 'Lector', hint: 'Solo lectura en toda la app' },
];
...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(`Se actualizó ${member.full_name || 'el miembro'} a ${nextRole}`);
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      toast.success(`Se quitó a ${removingMember.full_name || 'el miembro'}`);
      setMembers((prev) =>
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      <SettingsPanelHead
        ...
        description="Personas con acceso a esta cuenta. Los roles controlan lo que puede hacer cada compañero."
        action={
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              Por seguridad, la URL de invitación sin cifrar solo se
              muestra una vez al crearla — para volver a compartirla,
              revoca la invitación de abajo y crea una nueva.
            </p>
          ) : null}
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ...
                  arriba para generar un enlace para compartir.
                </p>
              </CardContent>
            </Card>
          ) : (
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              de la cuenta? Cerrará sesión en esta cuenta y recibirá una
              cuenta personal nueva en su próximo inicio de sesión. Su
