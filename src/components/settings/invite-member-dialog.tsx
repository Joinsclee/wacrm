...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our JoinsClee WaCrm account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'our JoinsClee WaCrm account';
    const message = `Join ${accountName} on JoinsClee WaCrm using this link (valid for ${result?.expiresInDays} days): ${url}`;
